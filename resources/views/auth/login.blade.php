    ...
    <div class="relative mx-auto mt-10 max-w-md sm:mt-16">
        ...
        <div class="stagger-item app-card space-y-6 p-6 sm:p-8 transition-all duration-500 hover:border-brand-500/30">
            <div class="text-center">
                ...
                <h1 class="stagger-item mt-4 text-2xl font-bold tracking-tight text-white" style="animation-delay:120ms">{{ __('Welcome back') }}</h1>
                <p class="stagger-item mt-2 text-sm text-slate-400" style="animation-delay:200ms">{{ __('Internal access for administrators and sales teams.') }}</p>
            </div>
...
<script>
    (function () {
        ...
        function refreshToken() {
            fetch('{{ route('auth.refresh-token') }}', { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
                .then(response => {

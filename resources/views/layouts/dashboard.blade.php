...
@php
    ...
    $crmModules = [
        ...
        ['key' => 'whatsapp', 'label' => __('Social Messages'), 'route' => 'dashboard.whatsapp.index', 'tint' => 'bg-emerald-500/15 text-emerald-400', 'icon' => '<svg class="h-4 w-4" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413z"/></svg>', 'permission' => 'view whatsapp|view all whatsapp conversations|manage crm'],
    ...
    $isSalesRoute = $currentRoute !== null && (
        str_starts_with($currentRoute, 'dashboard.sales-teams')
        || str_starts_with($currentRoute, 'dashboard.sales-targets')
        || str_starts_with($currentRoute, 'dashboard.sales-performance')
        || str_starts_with($currentRoute, 'dashboard.sales-evaluations')
        || str_starts_with($currentRoute, 'dashboard.automation')
    );
    $isSalesActive = fn ($prefix) => $currentRoute !== null && str_starts_with($currentRoute, 'dashboard.'.$prefix);
    $salesModules = [
        ['key' => 'sales-teams', 'label' => __('Teams'), 'route' => 'dashboard.sales-teams.index', 'permission' => 'manage teams', 'tint' => 'bg-brand-500/15 text-brand-400', 'icon' => '<svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" stroke-width="1.8"/><circle cx="9" cy="7" r="4" stroke-width="1.8"/><path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75" stroke-width="1.8"/><path d="M14.5 6a2.5 2.5 0 1 1-5 0 2.5 2.5 0 0 1 5 0Z" stroke-width="1.8"/></svg>'],
        ['key' => 'sales-targets', 'label' => __('Targets'), 'route' => 'dashboard.sales-targets.index', 'permission' => 'manage teams', 'tint' => 'bg-amber-500/15 text-amber-400', 'icon' => '<svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"><circle cx="12" cy="12" r="9" stroke-width="1.8"/><circle cx="12" cy="12" r="5" stroke-width="1.8"/><circle cx="12" cy="12" r="1.5" stroke-width="1.8"/></svg>'],
        ['key' => 'sales-performance', 'label' => __('Performance'), 'route' => 'dashboard.sales-performance.index', 'permission' => 'view reports', 'tint' => 'bg-emerald-500/15 text-emerald-400', 'icon' => '<svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M3 3v18h18" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/><path d="M7 15l4-5 3 3 5-7" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>'],
        ['key' => 'sales-evaluations', 'label' => __('Scorecard'), 'route' => 'dashboard.sales-evaluations.index', 'permission' => 'view reports', 'tint' => 'bg-violet-500/15 text-violet-400', 'icon' => '<svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M9 3h6M10 3v3.5a4 4 0 0 1-4 4v4a4 4 0 0 1 4 4V22M14 3v3.5a4 4 0 0 0 4 4v4a4 4 0 0 0-4 4V22" stroke-width="1.8" stroke-linecap="round"/></svg>'],
        ['key' => 'automation', 'label' => __('Automation'), 'route' => 'dashboard.automation.index', 'permission' => 'manage settings', 'tint' => 'bg-slate-500/15 text-slate-300', 'icon' => '<svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M13 2 3 14h9l-1 8 10-12h-9l1-8z" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>'],
    ];
@endphp
...
<body class="dash-ui min-h-full bg-slate-950 text-slate-100" x-data="{
    ...
    crmSubmenuOpen: {{ $isCrmRoute ? 'true' : 'false' }},
    crmFlyoutOpen: false,
    salesSubmenuOpen: {{ $isSalesRoute ? 'true' : 'false' }},
    salesFlyoutOpen: false,
    flyoutTop: 0,
    flyoutTimer: null,
    openCrmFlyout(el) {
        ...
        this.salesFlyoutOpen = false; // never show two flyouts at once
        this.crmFlyoutOpen = true;
    },
    closeCrmFlyout() {
        if (!this.sidebarCollapsed) return;
        this.flyoutTimer = setTimeout(() => { this.crmFlyoutOpen = false; }, 500);
    },
    keepCrmFlyoutOpen() {
        clearTimeout(this.flyoutTimer);
        this.crmFlyoutOpen = true;
    ...
    openSalesFlyout(el) {
        ...
        clearTimeout(this.flyoutTimer);
        this.flyoutTop = el.getBoundingClientRect().top - el.closest('aside').getBoundingClientRect().top;
        this.crmFlyoutOpen = false; // never show two flyouts at once
        this.salesFlyoutOpen = true;
    },
    closeSalesFlyout() {
        if (!this.sidebarCollapsed) return;
        this.flyoutTimer = setTimeout(() => { this.salesFlyoutOpen = false; }, 500);
    },
    keepSalesFlyoutOpen() {
        clearTimeout(this.flyoutTimer);
        this.salesFlyoutOpen = true;
    ...
    <!-- Desktop Sidebar -->
    <aside class="hidden lg:flex fixed inset-y-0 ltr:left-0 rtl:right-0 flex-col bg-slate-950/80 backdrop-blur-xl border-x border-white/10 z-50 transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] h-screen" :class="sidebarCollapsed ? 'w-[76px]' : 'w-72'">
        <div class="flex h-16 shrink-0 items-center border-b border-white/10 transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]" :class="sidebarCollapsed ? 'justify-center px-2' : 'px-6'">
            ...
            <div x-show="sidebarCollapsed" x-cloak x-transition.scale.origin.center.duration.300ms>
                <span class="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-brand-500 to-violet-600 text-sm font-black text-white shadow-lg shadow-brand-600/30">
                    {{ mb_strtoupper(mb_substr($companyProfile?->name ?? config('app.name'), 0, 1)) }}
                </span>
            </div>
        </div>
        <div class="flex-1 min-h-0 overflow-y-auto py-6 px-4 space-y-8 sidebar-scroll overscroll-contain">
            <div>
                ...
                <nav class="space-y-1">
                    ...
                    <div class="relative">
                        <button type="button" @click="if(!sidebarCollapsed) crmSubmenuOpen = !crmSubmenuOpen" @mouseenter="openCrmFlyout($el)" @mouseleave="closeCrmFlyout()" :title="sidebarCollapsed ? '{{ __('CRM') }}' : ''" class="flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-sm font-semibold transition {{ $isCrmRoute ? 'bg-brand-600 text-white shadow-lg shadow-brand-600/20' : 'text-slate-400 hover:bg-white/5 hover:text-white' }}" :class="sidebarCollapsed ? 'justify-center px-0' : ''">
                            ...
                            <span x-cloak :class="sidebarCollapsed ? 'max-w-0 opacity-0 flex-none' : 'max-w-52 opacity-100 flex-1'" class="inline-block overflow-hidden whitespace-nowrap text-start transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]">{{ __('CRM') }}</span>
                        ...
                        {{-- Expanded: dropdown submenu (internal scroll) --}}
                        <div x-show="!sidebarCollapsed && crmSubmenuOpen" x-cloak x-transition class="mt-1 space-y-0.5 overflow-y-auto overscroll-contain rounded-2xl border border-white/5 bg-white/5 p-1.5" style="max-height: min(60vh, 26rem); scrollbar-width: thin;">
                            @foreach ($crmModules as $module)
                                <a href="{{ route($module['route']) }}" class="flex items-center gap-2.5 rounded-xl px-2.5 py-2 text-[13px] font-semibold transition {{ $isCrmActive($module['key']) ? 'bg-brand-600 text-white' : 'text-slate-400 hover:bg-white/10 hover:text-white' }}">
                                    <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg {{ $module['tint'] }}">{!! $module['icon'] !!}</span>
                    ...
                    <div class="relative">
                        <button type="button" @click="if(!sidebarCollapsed) salesSubmenuOpen = !salesSubmenuOpen" @mouseenter="openSalesFlyout($el)" @mouseleave="closeSalesFlyout()" :title="sidebarCollapsed ? '{{ __('Sales Team') }}' : ''" class="flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-sm font-semibold transition {{ $isSalesRoute ? 'bg-brand-600 text-white shadow-lg shadow-brand-600/20' : 'text-slate-400 hover:bg-white/5 hover:text-white' }}" :class="sidebarCollapsed ? 'justify-center px-0' : ''">
                            ...
                            <span x-cloak :class="sidebarCollapsed ? 'max-w-0 opacity-0 flex-none' : 'max-w-52 opacity-100 flex-1'" class="inline-block overflow-hidden whitespace-nowrap text-start transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]">{{ __('Sales Team') }}</span>
                            <svg x-show="!sidebarCollapsed" x-cloak class="h-4 w-4 shrink-0 transition-transform duration-300" :class="salesSubmenuOpen ? 'rotate-180' : ''" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="m6 9 6 6 6-6" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                        </button>

                        {{-- Expanded: dropdown submenu --}}
                        <div x-show="!sidebarCollapsed && salesSubmenuOpen" x-cloak x-transition class="mt-1 space-y-0.5 overflow-y-auto overscroll-contain rounded-2xl border border-white/5 bg-white/5 p-1.5" style="max-height: min(60vh, 26rem); scrollbar-width: thin;">
                            @foreach ($salesModules as $module)
                                @can($module['permission'])
                                    <a href="{{ route($module['route']) }}" class="flex items-center gap-2.5 rounded-xl px-2.5 py-2 text-[13px] font-semibold transition {{ $isSalesActive($module['key']) ? 'bg-brand-600 text-white' : 'text-slate-400 hover:bg-white/10 hover:text-white' }}">
                                        <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg {{ $module['tint'] }}">{!! $module['icon'] !!}</span>
                                        <span class="flex-1 truncate">{{ $module['label'] }}</span>
                                    </a>
                                @endcan
            ...
            <div>
                ...
                <nav class="space-y-1">
                    ...
                    @can('manage settings')
                        ...
                        <a class="flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-semibold transition {{ $currentRoute === 'dashboard.settings.index' ? 'bg-brand-600 text-white shadow-lg shadow-brand-600/20' : 'text-slate-400 hover:bg-white/5 hover:text-white' }}" :class="sidebarCollapsed ? 'justify-center px-0' : ''" :title="sidebarCollapsed ? '{{ __('Settings') }}' : ''" href="{{ route('dashboard.settings.index') }}">
                            ...
                            <span x-cloak :class="sidebarCollapsed ? 'max-w-0 opacity-0' : 'max-w-52 opacity-100'" class="inline-block overflow-hidden whitespace-nowrap transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]">{{ __('Settings') }}</span>
                        </a>
                        <a class="flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-semibold transition {{ $currentRoute === 'dashboard.maintenance.index' ? 'bg-brand-600 text-white shadow-lg shadow-brand-600/20' : 'text-slate-400 hover:bg-white/5 hover:text-white' }}" :class="sidebarCollapsed ? 'justify-center px-0' : ''" :title="sidebarCollapsed ? '{{ __('Maintenance Tools') }}' : ''" href="{{ route('dashboard.maintenance.index') }}">
                            <svg class="h-5 w-5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M14.7 6.3a4.5 4.5 0 0 0-6 6L3 18l3 3 5.7-5.7a4.5 4.5 0 0 0 6-6L14.5 12l-2.5-2.5 2.7-3.2Z" stroke-width="1.8" stroke-linejoin="round"/></svg>
                            <span x-cloak :class="sidebarCollapsed ? 'max-w-0 opacity-0' : 'max-w-52 opacity-100'" class="inline-block overflow-hidden whitespace-nowrap transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]">{{ __('Maintenance Tools') }}</span>
                        </a>
                        <a class="flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-semibold transition {{ str_starts_with((string) $currentRoute, 'dashboard.email-templates') ? 'bg-brand-600 text-white shadow-lg shadow-brand-600/20' : 'text-slate-400 hover:bg-white/5 hover:text-white' }}" :class="sidebarCollapsed ? 'justify-center px-0' : ''" :title="sidebarCollapsed ? '{{ __('Email Templates') }}' : ''" href="{{ route('dashboard.email-templates.index') }}">
            ...
            <div>
                <p x-cloak :class="sidebarCollapsed ? 'max-w-0 opacity-0' : 'max-w-52 opacity-100'" class="overflow-hidden whitespace-nowrap px-2 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500 transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] mb-4">{{ __('Account') }}</p>
                <nav class="space-y-1">
                    <a href="{{ route('dashboard.profile.index') }}" :title="sidebarCollapsed ? '{{ __('My Profile') }}' : ''" class="flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-semibold transition {{ $currentRoute === 'dashboard.profile.index' ? 'bg-brand-600 text-white shadow-lg shadow-brand-600/20' : 'text-slate-400 hover:bg-white/5 hover:text-white' }}" :class="sidebarCollapsed ? 'justify-center px-0' : ''">
                        <svg class="h-5 w-5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" stroke-width="1.8"/><circle cx="9" cy="7" r="4" stroke-width="1.8"/><path d="M22 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75" stroke-width="1.8"/></svg>
                        <span x-cloak :class="sidebarCollapsed ? 'max-w-0 opacity-0' : 'max-w-52 opacity-100'" class="inline-block overflow-hidden whitespace-nowrap transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]">{{ __('My Profile') }}</span>
                    </a>
                </nav>
            </div>
            ...
            <div>
                ...
                <nav class="space-y-1">
                    <a href="{{ route('dashboard.installments.index') }}" :title="sidebarCollapsed ? '{{ __('Calculator') }}' : ''" class="flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-semibold transition text-slate-400 hover:bg-white/5 hover:text-white" :class="sidebarCollapsed ? 'justify-center px-0' : ''">
                        ...
                        <span x-cloak :class="sidebarCollapsed ? 'max-w-0 opacity-0' : 'max-w-52 opacity-100'" class="inline-block overflow-hidden whitespace-nowrap transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]">{{ __('Calculator') }}</span>
                    </a>
                </nav>
            </div>
        </div>
        <div class="p-3.5 mt-auto shrink-0 border-t border-white/5 space-y-2 bg-slate-950/40">
            <button type="button" @click="sidebarCollapsed = !sidebarCollapsed" :title="sidebarCollapsed ? '{{ __('Expand') }}' : '{{ __('Collapse') }}'" class="flex w-full items-center justify-center gap-2 rounded-2xl bg-white/5 py-3 text-sm font-bold text-slate-300 hover:bg-white/10 hover:text-white transition">
                ...
                <span x-cloak :class="sidebarCollapsed ? 'max-w-0 opacity-0' : 'max-w-52 opacity-100'" class="inline-block overflow-hidden whitespace-nowrap transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]">{{ __('Collapse') }}</span>
            </button>
            <a href="{{ route('home') }}" :title="sidebarCollapsed ? '{{ __('Public Website') }}' : ''" class="flex items-center justify-center gap-2 w-full py-3 rounded-2xl bg-white/5 text-sm font-bold text-white hover:bg-white/10 transition">
                ...
                <span x-cloak :class="sidebarCollapsed ? 'max-w-0 opacity-0' : 'max-w-52 opacity-100'" class="inline-block overflow-hidden whitespace-nowrap transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]">{{ __('Public Website') }}</span>
            </a>
        </div>

        {{-- Collapsed flyout — direct child of <aside> so it escapes the scroll container's overflow --}}
        <div x-show="sidebarCollapsed && crmFlyoutOpen" x-cloak x-transition.scale.origin.top.duration.300ms class="absolute z-[60] w-64 ltr:left-full rtl:right-full ltr:pl-1 rtl:pr-1" :style="{ top: flyoutTop + 'px' }" @mouseenter="keepCrmFlyoutOpen()" @mouseleave="closeCrmFlyout()">
            <div class="rounded-2xl border border-white/10 bg-slate-900 p-2 shadow-2xl shadow-black/60 backdrop-blur-xl" style="max-height: 70vh; overflow-y: auto; overscroll-behavior: contain; scrollbar-width: thin;">
                <p class="px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500">{{ __('CRM') }}</p>
                @foreach ($crmModules as $module)
                    <a href="{{ route($module['route']) }}" class="flex items-center gap-2.5 rounded-xl px-2.5 py-2 text-[13px] font-semibold transition {{ $isCrmActive($module['key']) ? 'bg-brand-600 text-white' : 'text-slate-400 hover:bg-white/10 hover:text-white' }}">
                        <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg {{ $module['tint'] }}">{!! $module['icon'] !!}</span>
                        <span class="flex-1 truncate">{{ $module['label'] }}</span>
        ...
        {{-- Sales Team collapsed flyout --}}
        <div x-show="sidebarCollapsed && salesFlyoutOpen" x-cloak x-transition.scale.origin.top.duration.300ms class="absolute z-[60] w-64 ltr:left-full rtl:right-full ltr:pl-1 rtl:pr-1" :style="{ top: flyoutTop + 'px' }" @mouseenter="keepSalesFlyoutOpen()" @mouseleave="closeSalesFlyout()">
            <div class="rounded-2xl border border-white/10 bg-slate-900 p-2 shadow-2xl shadow-black/60 backdrop-blur-xl" style="max-height: 70vh; overflow-y: auto; overscroll-behavior: contain; scrollbar-width: thin;">
                <p class="px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500">{{ __('Sales Team') }}</p>
                @foreach ($salesModules as $module)
                    @can($module['permission'])
                        <a href="{{ route($module['route']) }}" class="flex items-center gap-2.5 rounded-xl px-2.5 py-2 text-[13px] font-semibold transition {{ $isSalesActive($module['key']) ? 'bg-brand-600 text-white' : 'text-slate-400 hover:bg-white/10 hover:text-white' }}">
                            <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg {{ $module['tint'] }}">{!! $module['icon'] !!}</span>
    ...
    <div class="min-h-screen flex flex-col transition-all duration-300" :class="sidebarCollapsed ? 'ltr:lg:ml-[76px] rtl:lg:mr-[76px]' : 'ltr:lg:ml-72 rtl:lg:mr-72'">
        <header class="sticky top-0 z-40 border-b border-white/10 bg-slate-950/90 px-4 py-2.5 backdrop-blur-xl lg:px-6 lg:py-3">
            <div class="flex items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                    <button type="button" class="touch-target inline-flex items-center justify-center rounded-2xl bg-white/5 lg:hidden" @click="drawerOpen = true" aria-label="{{ __('Open menu') }}">
                    ...
                    <div class="hidden lg:block">
                        @include('partials.company-logo', ['size' => 'desktop'])
                    </div>
                    <div class="lg:hidden">
                        @include('partials.company-logo', ['size' => 'mobile'])
                    </div>
                    <div class="relative hidden sm:block lg:ml-4">
                        <form action="{{ route('dashboard.crm.search') }}" method="GET">
                            ...
                            <input type="text" name="q" value="{{ request('q') }}" class="h-9 w-64 rounded-xl border border-white/5 bg-white/5 pl-9 pr-4 text-sm text-slate-200 placeholder:text-slate-500 transition focus:border-brand-500/50 focus:bg-white/10 focus:ring-0 lg:w-80" placeholder="{{ __('Search...') }}">
                        </form>
                    </div>
                </div>

                <div class="flex items-center gap-2">
                    <a href="{{ route('dashboard.crm.search') }}" class="touch-target inline-flex h-10 w-10 items-center justify-center rounded-xl bg-white/5 text-slate-400 transition hover:text-white sm:hidden" aria-label="{{ __('Search') }}">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button type="button" class="touch-target hidden h-10 w-10 items-center justify-center rounded-xl bg-white/5 text-slate-400 hover:text-white transition lg:inline-flex" onclick="window.theme.toggle()" aria-label="{{ __('Toggle theme') }}">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open; if (open) positionUserMenu($el, $refs.panel)" type="button" class="relative touch-target inline-flex h-10 w-10 items-center justify-center rounded-xl bg-white/5 text-slate-400 transition hover:text-white" aria-label="{{ __('Notifications') }}">
                            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/><path d="M13.73 21a2 2 0 0 1-3.46 0" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/><circle cx="20" cy="4" r="3" fill="#22c55e" opacity="0" data-live="notif-dot" style="display:none"/></svg>
                            @if ($dashboardUnreadCount > 0)
                                <span data-live="notif-count" class="absolute -top-0.5 -right-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-rose-500 px-1 text-[10px] font-bold text-white shadow-lg shadow-rose-500/40">{{ $dashboardUnreadCount > 9 ? '9+' : $dashboardUnreadCount }}</span>
                            @endif
                        </button>
                        ...
                        <div x-show="open" @click.outside="open = false" x-cloak x-transition x-ref="panel" class="fixed top-12 z-[70] w-80 max-w-[calc(100vw-1rem)] rounded-2xl border border-white/10 bg-slate-900 shadow-2xl shadow-black/60 backdrop-blur-xl">
                            <div class="flex items-center justify-between gap-2 border-b border-white/5 px-4 py-3">
                                <p class="text-sm font-bold text-white">{{ __('Notifications') }}</p>
                                @if ($dashboardUnreadCount > 0)
                                    <form method="POST" action="{{ route('dashboard.notifications.read-all') }}">
                                        @csrf
                                        <button type="submit" class="text-xs font-semibold text-brand-300 transition hover:text-brand-200">{{ __('Mark all as read') }}</button>
                                    </form>
                                @endif
                            </div>
                            <div class="max-h-96 overflow-y-auto p-2">
                                @forelse ($dashboardNotifications as $notification)
                                    <a href="{{ $notification->url }}" @click="fetch('{{ route('dashboard.notifications.read', $notification->id) }}', { method: 'POST', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content }, keepalive: true })" class="flex gap-3 rounded-xl px-3 py-2.5 transition hover:bg-white/5 {{ $notification->unread ? 'bg-brand-500/10' : '' }}">
                                        <span class="mt-1.5 flex h-2 w-2 shrink-0 rounded-full {{ $notification->unread ? 'bg-rose-400' : 'bg-transparent' }}"></span>
                                        <span class="min-w-0">
                                            <span class="block text-sm font-semibold text-white">{{ $notification->title }}</span>
                                            <span class="mt-0.5 block text-xs leading-5 text-slate-400">{{ $notification->message }}</span>
                                            <span class="mt-1 block text-[11px] text-slate-500">{{ $notification->created_at?->diffForHumans() }}</span>
                                        </span>
                                    </a>
                                @empty
                                    <div class="px-3 py-8 text-center">
                                        <svg class="mx-auto h-8 w-8 text-slate-600" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/><path d="M13.73 21a2 2 0 0 1-3.46 0" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/><circle cx="20" cy="4" r="3" fill="#22c55e" opacity="0" data-live="notif-dot" style="display:none"/></svg>
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open; if (open) positionUserMenu($el, $refs.panel)" type="button" class="flex items-center gap-2 rounded-xl bg-white/5 p-1.5 transition hover:bg-white/10">
                            ...
                            <span class="hidden text-sm font-semibold text-white lg:block mr-1">{{ auth()->user()?->name }}</span>
                        ...
                        <div x-show="open" @click.outside="open = false" x-cloak x-transition x-ref="panel" class="fixed top-12 z-[70] w-56 max-w-[calc(100vw-1rem)] rounded-2xl border border-white/10 bg-slate-900 p-2 shadow-2xl shadow-black/60 backdrop-blur-xl">
                            <div class="px-3 py-2 border-b border-white/5 mb-1">
                                <p class="text-xs font-bold uppercase tracking-wider text-slate-500">{{ __('Account') }}</p>
                                <p class="text-sm font-semibold text-white truncate">{{ auth()->user()?->email }}</p>
                            </div>
                            <a href="{{ route('home') }}" class="flex items-center gap-2 rounded-xl px-3 py-2 text-sm text-slate-300 hover:bg-white/5 hover:text-white transition">
                            ...
                            <button type="button" onclick="PushManager.isEnabled() ? PushManager.unsubscribe() : PushManager.subscribe()" class="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-sm text-slate-300 hover:bg-white/5 hover:text-white transition text-left">
                                ...
                                <span id="push-toggle-label">{{ __('Enable Notifications') }}</span>
                                <span id="push-toggle-badge" class="ms-auto rounded-full px-2 py-0.5 text-[10px] font-bold" style="display:none;"></span>
                            </button>
                            <script>
                            document.addEventListener('push-status-changed', function(e) {
                                var label = document.getElementById('push-toggle-label');
                                var badge = document.getElementById('push-toggle-badge');
                                if (e.detail.enabled) {
                                    label.textContent = '{{ __('Disable Notifications') }}';
                                    badge.style.display = '';
                                    badge.className = 'ms-auto rounded-full bg-emerald-500/20 px-2 py-0.5 text-[10px] font-bold text-emerald-300';
                                    badge.textContent = '{{ __('On') }}';
                                } else {
                                    label.textContent = '{{ __('Enable Notifications') }}';
                                    badge.style.display = 'none';
                                }
                            });
                            </script>
                            <a href="{{ route('dashboard.security') }}" class="flex items-center gap-2 rounded-xl px-3 py-2 text-sm text-slate-300 hover:bg-white/5 hover:text-white transition">
                                ...
                                <span class="flex items-center gap-2">
                                    ...
                                    @if (auth()->user()?->two_factor_enabled)
                                        <span class="rounded-full bg-emerald-500/20 px-2 py-0.5 text-[10px] font-bold text-emerald-300">{{ __('On') }}</span>
                                    @endif
                                </span>
                            </a>
        ...
        @if (!request()->routeIs('dashboard.whatsapp.index', 'dashboard.whatsapp.reports', 'dashboard.whatsapp.templates.index'))
            @include('partials.mobile-fab')
        @endif
        @include('partials.mobile-bottom-nav')

        <div x-cloak x-show="drawerOpen" class="mobile-drawer__overlay" @click="drawerOpen = false"></div>
        <aside x-cloak x-show="drawerOpen" x-transition class="mobile-drawer flex flex-col h-full max-h-screen overflow-hidden">
            <div class="flex shrink-0 items-center justify-between gap-3 border-b border-white/10 pb-4">
                <div>
                    <p class="text-xs uppercase tracking-[0.2em] text-slate-400">{{ __('Signed in as') }}</p>
                    <p class="text-lg font-semibold text-white">{{ auth()->user()?->name }}</p>
                    <p class="truncate text-xs text-slate-500">{{ auth()->user()?->email }}</p>
                </div>
                <button type="button" class="touch-target rounded-full bg-white/5" @click="drawerOpen = false" aria-label="{{ __('Close menu') }}">
            ...
            <div class="mt-3 flex-1 min-h-0 overflow-y-auto space-y-2 py-2 pr-1 sidebar-scroll overscroll-contain pb-8">
                ...
                @canany(['manage teams', 'view reports', 'manage settings'])
                    ...
                    @foreach ($salesModules as $module)
                        @can($module['permission'])
                            <a class="mobile-drawer__link {{ $isSalesActive($module['key']) ? 'is-active' : '' }}" href="{{ route($module['route']) }}">
                                <span class="mobile-drawer__link-content"><span class="mobile-drawer__icon">{!! $module['icon'] !!}</span>{{ $module['label'] }}</span><span>→</span>
                            </a>
                        @endcan
                    @endforeach
                @endcanany
                ...
                @can('manage settings')
                    ...
                    <a class="mobile-drawer__link {{ $currentRoute === 'dashboard.settings.index' ? 'is-active' : '' }}" href="{{ route('dashboard.settings.index') }}">
                        <span class="mobile-drawer__link-content"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M12.22 2h-.44a2 2 0 0 0-2 2v.18a2 2 0 0 1-1 1.73l-.43.25a2 2 0 0 1-2 0l-.15-.08a2 2 0 0 0-2.73.73l-.22.38a2 2 0 0 0 .73 2.73l.15.1a2 2 0 0 1 1 1.72v.51a2 2 0 0 1-1 1.74l-.15.09a2 2 0 0 0-.73 2.73l.22.38a2 2 0 0 0 2.73.73l.15-.08a2 2 0 0 1 2 0l.43.25a2 2 0 0 1 1 1.73V20a2 2 0 0 0 2 2h.44a2 2 0 0 0 2-2v-.18a2 2 0 0 1 1-1.73l.43-.25a2 2 0 0 1 2 0l.15.08a2 2 0 0 0 2.73-.73l.22-.39a2 2 0 0 0-.73-2.73l-.15-.08a2 2 0 0 1-1-1.74v-.5a2 2 0 0 1 1-1.74l.15-.09a2 2 0 0 0 .73-2.73l.22-.38a2 2 0 0 0-2.73-.73l-.15.08a2 2 0 0 1-2 0l-.43-.25a2 2 0 0 1-1-1.73V4a2 2 0 0 0-2-2z" stroke-width="1.8"/><circle cx="12" cy="12" r="3" stroke-width="1.8"/></svg>{{ __('Settings') }}</span><span>→</span>
                    </a>
                    <a class="mobile-drawer__link {{ $currentRoute === 'dashboard.maintenance.index' ? 'is-active' : '' }}" href="{{ route('dashboard.maintenance.index') }}">
                        <span class="mobile-drawer__link-content"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M14.7 6.3a4.5 4.5 0 0 0-6 6L3 18l3 3 5.7-5.7a4.5 4.5 0 0 0 6-6L14.5 12l-2.5-2.5 2.7-3.2Z" stroke-width="1.8"/></svg>{{ __('Maintenance Tools') }}</span><span>→</span>
                    </a>
                    <a class="mobile-drawer__link {{ str_starts_with((string) $currentRoute, 'dashboard.email-templates') ? 'is-active' : '' }}" href="{{ route('dashboard.email-templates.index') }}">
    ...
    {{-- Live Refresh: smart auto-refresh for dashboard --}}
    <style>
      .live-dot { width:6px; height:6px; border-radius:50%; background:#22c55e; display:inline-block; animation:live-blink 2s infinite; }
      .live-pulse { animation:live-flash .6s ease; }
      @keyframes live-blink { 0%,100%{opacity:1} 50%{opacity:.3} }
      @keyframes live-flash { 0%{transform:scale(1.3);color:#22c55e} 100%{transform:scale(1)} }
    </style>
    <script src="{{ asset('assets/js/live-refresh.js') }}" defer></script>
    <script src="{{ asset('assets/js/push-subscribe.js') }}" defer></script>

    {{-- PWA Install Banner (mobile only) --}}
    <div id="pwa-install-banner" class="fixed bottom-0 inset-x-0 z-[90] p-3 sm:hidden" style="display:none;">
      <div class="flex items-center gap-3 rounded-2xl border border-emerald-500/30 bg-gradient-to-r from-emerald-900/90 via-slate-900/95 to-slate-900/95 px-4 py-3 shadow-2xl shadow-emerald-900/40 backdrop-blur-xl">
        <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-emerald-500/20 text-emerald-400">
        ...
        <div class="min-w-0 flex-1">
          <p class="text-sm font-bold text-white">{{ __('Install App') }}</p>
          <p class="text-[11px] text-emerald-200/70">{{ __('Add to home screen for quick access') }}</p>
        </div>
        <button onclick="(async()=>{if(window.pwaPromptInstall){await window.pwaPromptInstall();}document.getElementById('pwa-install-banner').style.display='none';localStorage.setItem('pwa_install_dismissed','1');})()" class="shrink-0 rounded-xl bg-emerald-500 px-3.5 py-2 text-xs font-bold text-white shadow-lg shadow-emerald-500/30 transition active:scale-95">{{ __('Install') }}</button>
        <button onclick="document.getElementById('pwa-install-banner').style.display='none';localStorage.setItem('pwa_install_dismissed','1');" class="shrink-0 rounded-lg p-1.5 text-slate-400 hover:text-white transition">
          <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6 6 18M6 6l12 12" stroke-linecap="round"/></svg>
        </button>
      </div>
    </div>
    <script>
    (function(){
      ...
      if(!('ontouchstart' in window)||navigator.standalone)return;
      window.addEventListener('pwa-install-ready',function(){

        ...
        <div class="flex-1 overflow-y-auto py-6 px-4 space-y-8">
            <div>
                ...
                <nav class="space-y-1">
                    ...
                    <div class="relative">
                        <button type="button" @click="crmSubmenuOpen = !crmSubmenuOpen" @mouseenter="openCrmFlyout($el)" @mouseleave="closeCrmFlyout()" :title="sidebarCollapsed ? '{{ __('CRM') }}' : ''" class="flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-sm font-semibold transition {{ $isCrmRoute ? 'bg-brand-600 text-white shadow-lg shadow-brand-600/20' : 'text-slate-400 hover:bg-white/5 hover:text-white' }}" :class="sidebarCollapsed ? 'justify-center px-0' : ''">
            ...
            <div>
                ...
                <nav class="space-y-1">
                    ...
                    @endcan
                    @hasanyrole(['Administrator', 'Sales Manager'])
    ...
    <div class="min-h-screen flex flex-col transition-all duration-300" :class="sidebarCollapsed ? 'ltr:lg:ml-[76px] rtl:lg:mr-[76px]' : 'ltr:lg:ml-72 rtl:lg:mr-72'">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open; if (open) positionUserMenu($el, $refs.panel)" type="button" class="relative touch-target inline-flex h-10 w-10 items-center justify-center rounded-xl bg-white/5 text-slate-400 transition hover:text-white" aria-label="{{ __('Notifications') }}">
                            ...
                            @if ($dashboardUnreadCount > 0)
                                <span class="absolute -top-0.5 -right-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-rose-500 px-1 text-[10px] font-bold text-white shadow-lg shadow-rose-500/40">{{ $dashboardUnreadCount > 9 ? '9+' : $dashboardUnreadCount }}</span>
                            @endif
                        </button>
        ...
        <aside x-cloak x-show="drawerOpen" x-transition class="mobile-drawer">
            ...
            <div class="mt-4 space-y-2">
                <a class="mobile-drawer__link {{ $currentRoute === 'dashboard.home' ? 'is-active' : '' }}" href="{{ route('dashboard.home') }}">{{ __('Dashboard') }} <span>→</span></a>
                <a class="mobile-drawer__link {{ in_array($currentRoute, ['dashboard.crm.index', 'dashboard.crm.deals.index', 'dashboard.crm.deals.show']) ? 'is-active' : '' }}" href="{{ route('dashboard.crm.index') }}">{{ __('CRM') }} <span>→</span></a>
                <a class="mobile-drawer__link {{ $currentRoute === 'dashboard.crm.deals.index' ? 'is-active' : '' }}" href="{{ route('dashboard.crm.deals.index') }}">{{ __('Deals') }} <span>→</span></a>
                @can('view reports')
                    <a class="mobile-drawer__link {{ $currentRoute === 'dashboard.reports.index' ? 'is-active' : '' }}" href="{{ route('dashboard.reports.index') }}">{{ __('Reports & Insights') }} <span>→</span></a>
                    <a class="mobile-drawer__link {{ $currentRoute === 'dashboard.analytics.index' ? 'is-active' : '' }}" href="{{ route('dashboard.analytics.index') }}">{{ __('Site Analytics') }} <span>→</span></a>
                @endcan
                @can('manage settings')
                    <a class="mobile-drawer__link {{ str_starts_with((string) $currentRoute, 'dashboard.banners') ? 'is-active' : '' }}" href="{{ route('dashboard.banners.index') }}">{{ __('Banners') }} <span>→</span></a>
                @endcan
                @hasanyrole(['Administrator', 'Sales Manager'])
                    <a class="mobile-drawer__link {{ str_starts_with((string) $currentRoute, 'dashboard.trash') ? 'is-active' : '' }}" href="{{ route('dashboard.trash.index') }}">{{ __('Trash') }} <span>→</span></a>
                @endhasanyrole
                @can('manage users')
                    <a class="mobile-drawer__link {{ $currentRoute === 'dashboard.users.index' ? 'is-active' : '' }}" href="{{ route('dashboard.users.index') }}">{{ __('Users & Permissions') }} <span>→</span></a>
                @endcan
                @can('manage settings')
                    <a class="mobile-drawer__link {{ $currentRoute === 'dashboard.settings.index' ? 'is-active' : '' }}" href="{{ route('dashboard.settings.index') }}">{{ __('Settings') }} <span>→</span></a>
                @endcan
                <a class="mobile-drawer__link" href="{{ route('home') }}">{{ __('Public Website') }} <span>→</span></a>
                <a class="mobile-drawer__link" href="{{ route('dashboard.installments.index') }}">{{ __('Calculator') }} <span>→</span></a>

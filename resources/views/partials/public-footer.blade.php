...
<footer class="mt-20 border-t border-white/10 bg-slate-950/80">
    <div class="mx-auto w-full max-w-7xl px-4 py-12 lg:px-6">
        <div class="grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div class="space-y-3">
                <p class="mobile-section-title">{{ __('Quick Links') }}</p>
                <ul class="space-y-2.5 text-sm text-slate-400">
                    <li><a href="{{ route('public.projects.index') }}" class="transition hover:text-white">{{ __('المشاريع والوحدات المتاحة') }}</a></li>
                    <li><a href="{{ route('public.about') }}" class="transition hover:text-white">{{ __('من نحن') }}</a></li>
                    <li><a href="{{ route('public.contact') }}" class="transition hover:text-white">{{ __('اتصل بنا') }}</a></li>
                    <li><a href="{{ route('login') }}" class="transition hover:text-white">{{ __('بوابة مبيعات الشركة') }}</a></li>
                </ul>
            </div>

            <div class="space-y-3">
                <p class="mobile-section-title">{{ __('تواصل معنا') }}</p>
                <p class="text-sm leading-relaxed text-slate-400">
                    {{ __('احسب خطة سدادك بنفسك أو تواصل مع مسؤولي المبيعات للحصول على استشارة عقارية مخصصة.') }}
                </p>
                @if ($companyProfile?->address)
                    <p class="text-sm text-slate-400">{{ $companyProfile->address }}</p>
                @endif
            </div>
        </div>
        ...
        <div class="mt-12 flex flex-col items-center justify-between gap-3 border-t border-white/10 pt-6 sm:flex-row text-xs text-slate-500">
            <p>© {{ date('Y') }} <strong>{{ $companyProfile?->name ?? 'Venecia Developments' }}</strong> — {{ __('All rights reserved.') }}</p>
            <p>{{ __('صُنِعَ لأعلى مستويات الفخامة والتميز.') }}</p>
        </div>
    </div>
</footer>
